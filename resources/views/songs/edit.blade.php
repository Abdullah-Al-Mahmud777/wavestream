                ...
                <div class="card-body p-4">
                    ...
                    <form action="{{ route('songs.update', $song) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control bg-dark text-white border-secondary @error('album') is-invalid @enderror"
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select bg-dark text-white border-secondary @error('category_id') is-invalid @enderror"
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" {{ old('category_id', $song->category_id) == $category->id ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            @error('audio_file')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <small class="text-muted">Current file: {{ basename($song->file_path) }}</small>
                            <small class="text-muted d-block">Maximum file size: 10MB</small>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            @enderror
                            @if($song->cover_image)
                                <small class="text-muted">Current image: {{ basename($song->cover_image) }}</small>
                            @endif
                            <small class="text-muted d-block">Maximum file size: 2MB</small>
                        </div>

                        <div class="d-flex justify-content-between">
                            <a href="{{ route('songs.show', $song) }}" class="btn btn-outline-secondary">
